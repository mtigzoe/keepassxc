...
QString controlTypeName(CONTROLTYPEID type)
{
    ...
    case UIA_RadioButtonControlTypeId: return QStringLiteral("RadioButton");
    case UIA_SliderControlTypeId: return QStringLiteral("Slider");
    case UIA_SpinnerControlTypeId: return QStringLiteral("Spinner");
    case UIA_TabItemControlTypeId: return QStringLiteral("TabItem");
    case UIA_TreeControlTypeId: return QStringLiteral("Tree");
...
QString elementFrameworkId(IUIAutomationElement* element)
{
    if (!element) return {};
    BSTR value = nullptr;
    if (FAILED(element->get_CurrentFrameworkId(&value)) || !value) return {};
    const QString result = QString::fromWCharArray(value);
    SysFreeString(value);
    return result;
}
