...
void TagView::contextMenuRequested(const QPoint& pos)
{
    ...
    if (type == TagModel::SAVED_SEARCH) {
        ...
        QMenu menu;
        auto action = menu.exec({new QAction(icons()->icon("trash"), tr("Remove Search"), &menu)}, mapToGlobal(pos));
        if (action) {
        ...
        QMenu menu;
        auto action = menu.exec({new QAction(icons()->icon("trash"), tr("Remove Tag"), &menu)}, mapToGlobal(pos));
        if (action) {
