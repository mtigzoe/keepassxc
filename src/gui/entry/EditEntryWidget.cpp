...
void EditEntryWidget::setForms(Entry* entry, bool restore)
{
    ...
    m_mainUi->tagsList->setReadOnly(m_history);
...
void EditEntryWidget::displayAttribute(QModelIndex index, bool showProtected)
{
    ...
    if (index.isValid()) {
        ...
        if (showProtected) {
            ...
            // focus, move focus to the enabled Reveal button before disabling
            // the editor so keyboard and screen-reader focus remains usable.
            const bool attributesEditHasFocus = m_advancedUi->attributesEdit->hasFocus();
...
void EditEntryWidget::updateAutoTypeEnabled()
{
    ...
    // Redirect focus before disabling a focused Auto-Type control. This is
    // important for keyboard and screen-reader users because a disabled
    // widget is no longer an interactive keyboard target.
