...
void ReportsWidgetStatistics::addStatsRow(QString name, QString value, bool bad, QString badMsg)
{
    ...
    if (bad) {
        ...
        if (!badMsg.isEmpty()) {
            ...
            // The warning explanation must not depend on mouse-hover tooltips.
            // Expose it through the item's accessible description so JAWS/NVDA
            // users can receive the same information while navigating the table.
